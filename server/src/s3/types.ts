...
export const AwsAccountSchema = z.object({
  ...
  client_secret: z
    .string()
    .min(1, { message: 'client_secret cannot be empty' }),
...
export const AwsAccountsListSchema = z
  .array(AwsAccountSchema)
  .min(1, { message: 'No account configurations found' });
...
export const FileRenameOrCopyRequest = z
  .object({
    name: z.string().min(1),
    key: z.string().min(1),
    new_name: z.string().min(1),
  })
  .refine((obj) => obj.name !== obj.new_name, {
    message: 'new name cannot be same as current name',
  });
...
export const FileCopyRequest = z
  .object({
    name: z.string().min(1),
    key: z.string().min(1),
    new_name: z.string().min(1),
...
export const FileMoveRequest = z.object({
  name: z.string().min(1),
  key: z.string().min(1),
  destination: z.string().optional(),
});
