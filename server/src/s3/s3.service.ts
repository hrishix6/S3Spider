...
export class S3Service {
  ...
  async createBucket(
    ...
    try {
      ...
      const input = new CreateBucketCommand({
        Bucket: bucket,
        CreateBucketConfiguration: { LocationConstraint: region },
      });
      const created = await client.send(input);
      if (`${created.$metadata.httpStatusCode}`.startsWith('2')) {
        success = true;
        data = region;
      } else {
        success = false;
  ...
  async listBuckets(accountId: string) {
    ...
    try {
      ...
      const locationRequests = buckets.map((bucket) => {
        const locationInput = new GetBucketLocationCommand({
          Bucket: bucket.name,
        });
  ...
  async getSignedUrlForDL(
    ...
    try {
      ...
      const signedUrl = await getSignedUrl(client, dlCommand, {
        expiresIn: 300,
      });
      success = true;
      data = signedUrl;
  ...
  async getSignedUrlForUL(
    ...
    try {
      ...
      const signedUrl = await getSignedUrl(client, ulCommand, {
        expiresIn: 300,
      });
      success = true;
      data = signedUrl;
  ...
  /**
   * Creates a copy of an object in given destination.
   * @param accountId aws account
   * @param region bucket region
   * @param bucket bucket
  ...
  async copyObject(
    ...
    try {
      ...
      const newKey = `${destination}${newName}`;

      console.log(`copying from ${key} to ${newKey}`);
  ...
  /**
   * Creates a copy of an object in destination and delete original
   * @param accountId aws account
   * @param region bucket region
   * @param bucket bucket
   * @param key key of object being copied
  ...
  async moveObject(
    ...
    try {
      ...
      if (!CopyObjectResult) {
        ...
        const deleted = await this.deleteObjects(accountId, region, bucket, [
          key,
        ]);
        if (deleted.success) {
          console.log(`original file deleted`);
        } else {
          console.log(`couldn't delete original file`);
        }
        success = true;
        error = undefined;
      }
