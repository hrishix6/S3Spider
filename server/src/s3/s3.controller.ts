...
export class S3Controller extends BaseController {
  ...
  getBuckets: RequestHandler = async (req, res) => {
    ...
    const result = await this.s3CacheProxy.listBuckets(
      accountId,
      shouldIngoreCache
    );
  ...
  getPresignedUrlsForUL: RequestHandler = async (req, res) => {
    ...
    const result = await this.s3Service.getSignedUrlForUL(
      accountId,
      regionParse.data,
      bucketParse.data,
      keyParse.data
  ...
  getPresignedUrlsForDL: RequestHandler = async (req, res) => {
    ...
    const signedUrlRequests = files.map((x) =>
      this.s3Service.getSignedUrlForDL(
        accountId,
        regionParse.data,
        bucketParse.data,
        x.key,
  ...
  deleteFiles: RequestHandler = async (req, res) => {
    ...
    const result = await this.s3Service.deleteObjects(
      accountId,
      region,
      bucket,
      keys
  ...
  renameFile: RequestHandler = async (req, res) => {
    ...
    const result = await this.s3Service.renameObject(
      accountId,
      regionParse.data,
      bucketParse.data,
      key,
  ...
  copyFile: RequestHandler = async (req, res) => {
    ...
    const bodyParse = await FileCopyRequest.safeParseAsync(body);

    if (!bodyParse.success) {
      return this.badRequest(res, AppErrorCode.BAD_COPY_REQ);
    }

    const { new_name, key, destination } = bodyParse.data;

    const result = await this.s3Service.copyObject(
      accountId,
      regionParse.data,
      bucketParse.data,
      key,
  ...
  moveFile: RequestHandler = async (req, res) => {
    const { accountId } = req.params;
    ...
    const bodyParse = await FileMoveRequest.safeParseAsync(body);

    if (!bodyParse.success) {
      return this.badRequest(res, AppErrorCode.BAD_COPY_REQ);
    }

    const { name, key, destination } = bodyParse.data;

    const result = await this.s3Service.moveObject(
      accountId,
      regionParse.data,
      bucketParse.data,
      key,
  ...
  createFolder: RequestHandler = async (req, res) => {
    ...
    const result = await this.s3Service.createFolder(
      accountId,
      regionParse.data,
      bucketParse.data,
      `${key || ''}${name}/`
  ...
  routes() {
    ...
    router
      .route('/:accountId/buckets')
      .get(
        this.middlewares.awsAccountGuard.bind(this.middlewares),
        AsyncHandler(this.getBuckets.bind(this))
      );

    router
      .route('/:accountId/files')
      .get(
        this.middlewares.awsAccountGuard.bind(this.middlewares),
        AsyncHandler(this.getChildren.bind(this))
      ) //get
      .put(
        this.middlewares.awsAccountGuard.bind(this.middlewares),
        AsyncHandler(this.renameFile.bind(this))
      ) //rename
      .post(
        this.middlewares.awsAccountGuard.bind(this.middlewares),
        AsyncHandler(this.copyFile.bind(this))
      ); //copy

    router
      .route('/:accountId/folders')
      .post(
        this.middlewares.awsAccountGuard.bind(this.middlewares),
        AsyncHandler(this.createFolder.bind(this))
      ) //create folder
      .delete(
        this.middlewares.awsAccountGuard.bind(this.middlewares),
        AsyncHandler(this.deleteFolder.bind(this))
      ); //delete folder

    router.post(
      '/:accountId/files/rm',
      this.middlewares.awsAccountGuard.bind(this.middlewares),
      AsyncHandler(this.deleteFiles.bind(this))
    );

    router.post(
      '/:accountId/files/mv',
      this.middlewares.awsAccountGuard.bind(this.middlewares),
      AsyncHandler(this.moveFile.bind(this))
    );

    router.post(
      '/:accountId/files/dl',
      this.middlewares.awsAccountGuard.bind(this.middlewares),
      AsyncHandler(this.getPresignedUrlsForDL.bind(this))
    ); //download
